...
| **Postgres itself** | 🟡 **started.** The declaration emits Postgres DDL and `EVE_DATABASE_URL` is the seam. All 38 `INSERT OR REPLACE` statements are now `ON CONFLICT ... DO UPDATE`, which runs on both dialects; `strftime` is gone from queries and the pragma set is pinned. What remains is moving the call sites: ~316 statements use `?` placeholders and a raw `sqlite3.Connection`, and psycopg wants `%s`. `app/db/conn.py` is the seam and is proven against both backends; the schema, the migrations and the upsert helper all run on Postgres (`tests/test_postgres_schema.py`). The flip was written down as atomic — `get_conn()` is shared by every module — but it is not: a converted module takes its connection from `app.db.conn.connect()` while the rest still calls `get_conn()`, and both work on one database at once. So it goes module by module; `projects` is converted and is the worked example. W6 still earned its place first — those 105 statements are now spread across eleven reviewable routers rather than one 7,000-line file. |
...
| **Background sync worker** | 🟡 **event model decided and built; the loop is not.** `sync_events` is an append-only table with a monotonic id, and `app/sync/events.py` is the emit/consume API. A consumer keeps a cursor and asks for `id > cursor`, which answers "what did I miss while I was down?" exactly — the property the bot in §9.5 needs and the one a cache cannot provide, since two changes between polls look like one and a change that reverts looks like none. Nothing else considered has that property on both backends: an in-process signal dies with the process and is invisible to a second one, and LISTEN/NOTIFY is Postgres-only. NOTIFY is a good wake-up to add later; the log stays the source of truth underneath it. `emit()` deliberately does not commit, so an event lands in the same transaction as the cache write it describes. |
